# Percent Admin Phase 5 — backend change handoff

Date: 2026-09-21  
Target: Percent (`gijyjdeohvdrnvqfqdha`, `ap-south-1`)  
Proposed migration: `supabase/migrations/20260921120000_admin_customer_intelligence.sql`

## Why this change is required

The authoritative identity is already `auth.users.id = public.profiles.id`. The current account email exists only in `auth.users`, which is intentionally unavailable through the public API. Existing Admin RLS permits direct reads of profiles, addresses, orders, order snapshots, and reviews, but it cannot safely expose current Auth email. Building the list from separate browser queries would also require unbounded client joins or N+1 requests.

The smallest safe addition is two read-only, admin-gated RPCs:

- `admin_list_customers` returns one bounded page, aggregate metrics, search/filter/sort results, and current Auth email.
- `admin_get_customer` returns one customer with saved addresses, real orders, review history, wishlist information, and effective reservation summaries.

Both functions derive the caller from `auth.uid()`, require `private.is_admin()`, use a fixed empty `search_path`, fully qualify every database object, expose no generic SQL capability, and return customer records only. They do not create or mutate customer data.

## Metrics contract

- **Total Customers:** profiles whose existing private role is `customer`.
- **Customers With Orders:** customers with at least one recorded order in any lifecycle state.
- **New Customers:** customer profiles created in the current UTC calendar month.
- **Total Customer Order Value:** server-stored `total_paise` for orders whose order status is not `cancelled`.
- **Order Count:** all recorded orders, including cancelled orders, so account history is not hidden.
- **Lifetime/Average Order Value:** non-cancelled orders only.
- **Latest Order:** latest recorded order regardless of status.
...
begin
  ...
  with customer_base as materialized (
    ...
    join private.user_roles r on r.user_id = p.id and r.role = 'customer'
  ),
...
## Security and structural impact

- New tables: 0
- New columns: 0
- New schemas: 0
- New views: 0
- New read-only RPCs: 2
- Existing RLS policies: unchanged
- Direct access to `auth.users`: unchanged and unavailable to browser roles
- Direct access to other customers' profiles, addresses, orders, reviews, and wishlists: unchanged
- Anonymous execution: revoked
- Authenticated execution: granted, then restricted inside each function to `admin` or `super_admin` through `private.is_admin()`
- Mutations: none

After manual execution, Phase 5 should continue with hosted signature/grant verification, anon/customer/admin/super-admin role tests, exact aggregate checks against current hosted counts, and then the two Admin routes.
