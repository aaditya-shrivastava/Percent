-- Phase 5 customer eligibility refinement.
-- Include customer-role accounts and accounts with real customer activity,
-- while excluding staff-only accounts with no customer activity.
begin;
...
begin
  ...
  with customer_base as materialized (
    ...
    where r.role = 'customer'
       ...
       or exists (select 1 from public.wishlist_items wi where wi.user_id = p.id)
  ),
