-- Controlled one-way draft publication. The trigger prevents the existing
-- generic admin table grant from bypassing the authoritative readiness RPC.
begin;

create or replace function private.guard_product_publication()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if old.status in ('active','archived') and new.status = 'draft' then
    raise exception 'Published designs cannot return to draft' using errcode = '22023';
  end if;
  if old.status = 'draft' and new.status = 'active'
     and current_user not in ('postgres','supabase_admin') then
    raise exception 'Use publish_product for activation' using errcode = '42501';
  end if;
  return new;
end;
$$;

drop trigger if exists guard_product_publication on public.products;
...
revoke all on function private.guard_product_publication() from public, anon, authenticated, service_role;
