...
create function public.save_product_draft(draft jsonb, product_id uuid default null, expected_updated_at timestamptz default null)
...
begin
 ...
 if product_id is null then
  ...
  if expected_updated_at is null or p.updated_at is distinct from expected_updated_at then raise exception 'Product changed; reload before saving' using errcode='40001'; end if;
...
revoke all on function public.save_product_draft(jsonb,uuid,timestamptz) from public,anon,authenticated,service_role;
grant execute on function public.save_product_draft(jsonb,uuid,timestamptz) to authenticated;
