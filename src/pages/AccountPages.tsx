...
function ProfileContent() {
  ...
  const saveProfile = async () => {
    ...
    try { await updatePercentProfile(draft) } catch(e) { setErrors({ firstName: e instanceof Error ? e.message : 'Unable to save profile' }); return }
    setProfile(draft)
    setEditing(false)
  }
  ...
  return <>
    ...
    <section className="profile-information" aria-labelledby="personal-information-heading"><div className="account-section-title"><span>01</span><h2 id="personal-information-heading">Personal Information</h2></div><div className="profile-field-grid">
      {(['firstName', 'lastName', 'phone', 'email'] as const).map((field) => { const labels = { firstName: 'First Name', lastName: 'Last Name', phone: 'Phone Number', email: 'Email ID' }; return <label key={field}>{labels[field]}<input type={field === 'email' ? 'email' : field === 'phone' ? 'tel' : 'text'} readOnly={!editing || field === 'email'} value={(editing ? draft : profile)[field]} placeholder={field === 'phone' || field === 'email' ? 'Not added yet' : ''} aria-invalid={Boolean(errors[field])} onChange={(event) => setDraft((current) => ({ ...current, [field]: field === 'phone' ? event.target.value.replace(/\D/g, '').slice(0, 10) : event.target.value }))} />{errors[field] && <small>{errors[field]}</small>}</label> })}
...
export function ProfileAddressPage() {
  const { addresses, saveAddress, removeAddress, error: addressError } = useAccountAddresses()
  ...
  const submitAddress = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setErrors(nextErrors)
    if (Object.keys(nextErrors).length) return
    if (await saveAddress(draft)) setDraft(null)
  }
...
export function OrderDetailsPage() {
  ...
  if (loading) return <AccountShell><section className="account-empty" aria-busy="true"><Package /><h2>Loading Order…</h2></section></AccountShell>
    ...
    <div className="account-order-detail-layout">
      ...
      <aside className="account-order-detail-aside">
        ...
        {order.status !== 'Cancelled' && <section className="account-order-detail-card order-detail-fulfillment" aria-labelledby="order-fulfillment-heading">
          ...
          <strong>Shipping integration not connected</strong>
