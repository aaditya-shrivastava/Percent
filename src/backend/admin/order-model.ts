...
export function money(paise:number,currency='INR') { return new Intl.NumberFormat('en-IN',{style:'currency',currency,maximumFractionDigits:paise%100?2:0}).format(paise/100) }
export function orderItemCount(order:AdminOrder){return order.items.reduce((sum,item)=>sum+item.quantity,0)}
export function shippingAddress(order:AdminOrder){return order.addresses.find(a=>a.kind==='shipping')??order.addresses[0]}
export function orderCustomer(order:AdminOrder){const a=shippingAddress(order);return {name:a?.full_name??'Customer unavailable',email:a?.email??'',phone:a?.phone??''}}
